...
def main():
    ...
    if lensysargv < 2:
        print >>sys.stderr, "Split phylip file into given size with a slide"
        print >>sys.stderr, 'Using python %s phylip size[default 5] \
slide[default size, means no overlap. If a number less than size \
is given, the size of overlap is <size>-slide ]' % sys.argv[0]
        sys.exit(0)
    #-----------------------------------
    if lensysargv > 2:
        size = int(sys.argv[2])
    ...
    finalhead = [line[:pos], ' ' * blankN, str(last),
        line[pos+len(lengthstr)]]
...
#        finalhead = line[:pos] + str(last) + line[pos+len(lengthstr)]
    ...
    for i in range(0, length, slide):
        ...
        if end <= length:
            ...
            for item in keyL:
                print >>fh, \
                    "%s %s" % (item, aDict[item][start:end])
            fh.close()
        else:
            last = length - start
            blankN = len(lengthstr) - len(str(last))
            finalhead[1] = ' ' * blankN
            finalhead[2] = str(last)
            fh = open(output, 'w')
            print >>fh, ''.join(finalhead),
            for item in keyL:
                print >>fh, \
