...
def cntFile(file, test=0):
    ...
    baseD = {}
    '''
    baseD = {'A_percent':{0:2, 1:3}, 'T_percent':{0:3, 1:4}}
    '''
    len_line = 0
    read_in_cnt = 0
    for line in open(file):
        ...
        for i in range(7, len_line):
            percent = int(float(lineL[i]))
            if percent > 100:
                percent = 100
            key_110 = headerL[i]
            baseD[key_110][percent] = baseD[key_110].get(percent, 0)+cnt
    ...
    for key110, valueD in baseD.items():
        sum_reads_131 = sum(valueD.values())
        assert sum_reads_131 == sum_reads, file
        for key, value in valueD.items():
            valueD[key] = value/sum_reads_131*100
...
def output_base(baseD, frag_base_fh, label):
    '''
    baseD = {'A_percent':{0:2, 1:3}, 'T_percent':{0:3, 1:4}}
    "variable\tbase\tvalue\tlabel\tset"
    '''
    typeD = {'A':'Single base A content (%)', 'T':'Single base T content (%)', 
            'C':'Single base C content (%)', 
            'G':'Single base G content (%)', 'CpG':'Di-nucleotides CpG content (%)', 
            'CpG_ratio':'Di-nucleotides Observed to expected CpG ratio'}
    for key, valueD in baseD.items():
        key = key.replace('_percent', '')
        if key == 'CpG_observe_to_expect_ratio':
            key = "CpG_ratio"
        type = typeD[key]
        for percent, freq in valueD.items():
            print >>frag_base_fh, '{}\t{}\t{}\t{}'.format(percent, label, freq, type)
#---------------------------------------------------------------
