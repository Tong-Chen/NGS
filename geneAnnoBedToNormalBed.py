...
def main():
    ...
    for line in open(sys.argv[1]):
        ...
        for i in range(exonNum):
            ...
            print "%s\t%d\t%d\t%s%d\t0\t%s" % (lineL[0], start, end, \
                gene+r'.E.', i+1, lineL[5])
        #------intron---------------------
        start = 0
        for i in range(exonNum):
            ##usually for the first circulation, it will not 
            ##satisfy the if decision, so first intron will
            ##number 2.
            if start < exonStart[i]:
                start = chrSta + start
                end   = chrSta + exonStart[i] 
                print "%s\t%d\t%d\t%s%d\t0\t%s" % (lineL[0], start, end, \
                    gene+r'.I.', i+1, lineL[5])
            start = exonStart[i] + exonSize[i]
        if start < genesize:
                start = chrSta + start
                end   = chrSta + genesize
                print "%s\t%d\t%d\t%s%d\t0\t%s" % (lineL[0], start, end, \
                    gene+r'.I.', i+2, lineL[5])
        #----------------TSS-----------------------
