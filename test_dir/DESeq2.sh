...
sampleV
	...
	file <- paste("${file}${mid}", sampleA, sampleB,"results", sep=".")
	write.table(as.data.frame(res), file=file, sep="\t", quote=F, row.names=F)
	...
	file <- paste("${file}${mid}", 
		sampleA, sampleB,"results.DE_up", sep=".")
	write.table(as.data.frame(res_de_up), file=file, sep="\t", quote=F, row.names=F)
	res_de_up_id <- subset(res_de_up, select=c("ID"))
	file <- paste("${file}${mid}", 
		sampleA, sampleB,"results.DE_up_id", sep=".")
	write.table(as.data.frame(res_de_up_id), file=file, sep="\t", quote=F, row.names=F)
	
	res_de_dw <- subset(res_de, res_de\$log2FoldChange<=(-1)*${log2fc})
	file <- paste("${file}${mid}", 
		sampleA, sampleB,"results.DE_dw", sep=".")
	write.table(as.data.frame(res_de_dw), file=file, sep="\t", quote=F, row.names=F)
	res_de_dw_id <- subset(res_de_dw, select=c("ID"))
	file <- paste("${file}${mid}", 
		sampleA, sampleB,"results.DE_dw_id", sep=".")
	write.table(as.data.frame(res_de_dw_id), file=file, sep="\t", quote=F, row.names=F)
