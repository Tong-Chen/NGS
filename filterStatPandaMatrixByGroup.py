...
desc = '''
Program description:
    This is designed to extract part rows by given group, such as top 10 variable items in each group. 
'''
...
def cmdparameter(argv):
    ...
    parser.add_option("-r", "--index-col", dest="index_col",
        default='0', help="Column to use as the row labels of the DataFrame (Default 0 represents the first column). If a sequence like <0,1,2> is given, a MultiIndex will be used. Supply <None> to turn off index_col. All none numerical columns should be used as index.")
    parser.add_option("-g", "--group-col", dest="group_col",
        help="The column name used for grouping. Normally this column should be used as index also.")
...
#Supply <None> to disable rename.")
    ...
    parser.add_option("-t", "--top", dest="top",
        default='10', 
        help="Screen given number of top items or given percentage of top items. Default 10 for each group.")
...
def main():
    ...
    if top:
        ...
        if statistics_only:
            #matrix = matrix[0:top]
            matrix = matrix.groupby(group_col).head(top)
        else:
            matrix = matrix.groupby(group_col).head(top)
            dropCl = methods[:]
            dropCl.append(group_col)
            matrix = matrix.drop(dropCl, axis=1)
        if uncompressed:
            matrix.to_csv(output+'.top'+top_old+'.xls', sep=b"\t", index=False)
        else:
            matrix.to_csv(output+'.top'+top_old+'.xls.gz', compression='gzip', sep=b"\t", index=False)
    ###--------multi-process------------------
