...
## options
my %options=();
getopts("p:m:r:s:t:y:dokuc:nxg:e:f:vjwT:PWU:C",\%options);
...
[optional parameters]
  ...
  \t-C           The maximum allowed mapping loci when -U is used. Default 1.
...
sub ReadinReadsMappingFile{
	...
	#-------------------------------
	my $matchTime = 1;
	$matchTime = $options{'C'}+0 if(defined $options{'C'});
    ...
    while(<IN>){
		...
		next if($options{'U'} and $mapcounts{$line[0]} > $matchTime);
