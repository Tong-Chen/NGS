...
multipleColFile(tab separated) the special column(the column you want\
 ...
 a parameter given, use it as the missed value]' % sys.argv[0]
    ...
    for line in open(multiple):
        ...
        key = lineL[col]
        if key not in multipleDict:
            multipleDict[key] = [line]
        else:
            multipleDict[key].append(line)
    #------missing-----------------------------
    if auto_comp:
        auto_linel = lineL[:]
    ...
    for line in open(single):
        key = line.strip()
        if key in multipleDict:
             sys.stdout.writelines(multipleDict[key])
        else:
            if auto_comp:
                auto_linel[col] = key
                sys.stdout.writelines(auto_linel)
