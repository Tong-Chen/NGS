...
#                                        cond_B    cond_B_rep2
#
#  --FDR_thresh						FDR threshold for MA plot and Volcano plot. Default 0.05.	
#
#  --logFC_thresh					Log2 foldchange for MA and Volcano plot. Default 2 (meaning 4 times fold change).
#
#  General options:
#
...
&GetOptions ( 'h' => \$help_flag,
			  ...
			  'FDR_thresh=f' => \$FDR_thresh,
			  'logFC_thresh=f' => \$logFC_thresh,
...
main: {
    ...
    if ($method eq "GLM") {
        ...
        foreach my $DE_contrast (@DE_contrasts) {
            ...
            if ($method eq "edgeR") {
                &run_edgeR_sample_pair($matrix_file, \%samples, \%sample_name_to_column, $sample_a, $sample_b, $FDR_thresh, $logFC_thresh);
...
sub run_edgeR_sample_pair {
    my ($matrix_file, $samples_href, $sample_name_to_column_index_href, $sample_A, $sample_B, $FDR_thresh, $logFC_thresh) = @_;
    ...
    print $ofh "plot_MA(result_table\$logCPM, result_table\$logFC, result_table\$FDR, $FDR_thresh, $logFC_thresh)\n";
    ...
    print $ofh "plot_Volcano(result_table\$logFC, result_table\$FDR, $FDR_thresh, $logFC_thresh)\n";
