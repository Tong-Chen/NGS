...
Attention:
    1.If only start_codon or stop_codon exists (not both or all
...
will be output; Otherwise only outputting inner regions]' % sys.argv[0]
    ...
    parser.add_option("-i", "--input-file", dest="filein",
        metavar="FILEIN", help="Sorted GTF files (first by gene then \
by column 4 and 5). Normally one can sort file using sortGTF.py.")
    parser.add_option("-c", "--chrome-size", dest="chrome",
        help="Tab separated two columns file with chr_name and chr_length. \
Not necessary if no planning to extend TSS or TTS.")
    parser.add_option("-s", "--sep-symbol", dest="sep_symbol",
        default='.', help="A symbol used to separate different parts \
of name column in output bed file.")
...
def flankTSS_TES(TSS, TTS, end, regionL, strand, name_base, score, chr):
    ...
    for seg in regionL:
        ...
        if strand == '+':
            name = sep_symbol.join([name_base, 'TSS' + str(seg)])
        elif strand == '-':
            name = sep_symbol.join([name_base, 'TTS' + str(seg)])
        else:
...
def outTSS_TES(TSS, TTS, end, regionL, strand, name_base, score, chr):
    ...
    for seg in regionL:
        ...
        if newend > 0:
            if strand == '+':
                name = name_base + sep_symbol + 'TSS-UP' + str(oldseg) + '-' + str(seg)
            elif strand == '-':
                name = name_base + sep_symbol + 'TTS-DW' + str(oldseg) + '-' + str(seg)
            tmpLineL = [chr,str(newstart),str(newend),name, \
                score,strand]
            print "\t".join(tmpLineL)
        #------------TES---------------------------
        ...
        if strand == '+':
            if tmpDict[ele] == 'exon':
                ...
                if intron_start == -1:
                    ...
                    intron = sep_symbol.join([name, 'Intron',
                        str(intron_num)])
                    intron_num += 1
                ...
                if begin_coding_exon and not end_coding_exon:
                    exon = sep_symbol.join([name, 'Coding_exon',
                        str(exon_num)])
                    exon_num += 1
...
def main():
    ...
    #-----------------------------------
    file = options.filein
    chrom_size = options.chrome
    global sep_symbol
    sep_symbol = options.sep_symbol
