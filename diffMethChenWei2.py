    ...
    for line in fh:
        ...
        if header:
            ...
            for i in range(4, len_lineL, 4):
                typeL = ['C-F', 'D-F', "C-M", "D-M", 'C-FM', 'D-FM', 
                         'CD-FM', 'CD-FM-pvalue']
                sample = lineL[i].rsplit('_', 1)[0]
                keyL.extend([sample+'@'+i for i in typeL])
            print '\t'.join(keyL)
            continue
        #--------------------------------
        keyL = lineL[:4]
        #grp = 
        for i in range(4, len_lineL, 4):
            F, M, C, D = [float(lineL[i]) for i in range(i, i+4)]
            FM = (F+M)/2
            CD = (C+D)/2
            C_F = C - F
            D_F = D - F
            ...
            if diff >= 0.2 or diff <= -0.2:
                pvalue = ttest([F, M], [C, D])[1]
            else:
                pvalue = 1

            value = "%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.4f" % \
                    (C_F, D_F, C_M, D_M, C_FM, D_FM, diff, pvalue)
            keyL.append(value)
        #------------------------
        newLine = '\t'.join(keyL)
        print newLine
