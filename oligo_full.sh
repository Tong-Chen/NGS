...
$0 -f directory -s prefix${txtrst}

${bldblu}Function${txtrst}:

This script is used to analyze microarray data using <oligo> package.

${txtbld}OPTIONS${txtrst}:
	-f	Data directory
		${bldred}[Default current directory]${txtrst}
	...
	-T	Summary expression in which level.
		${bldred}Default probeset level, accept 'core' for gene arrays
		and 'core','full','extended' for exon arrays to summarize
		expresion at transcript level.${txtrst}
	-L	Support large datasets.
		${bldred}Default FALSE, accept TRUE. <ff> package is needed.${txtrst}
	-P	Parallize processing.
		${bldred}Default 1 meaning no parallize, accept an integer
		larger than 1 to specify the numberof cores one want to use. <ff, doMC> package is needed.${txtrst}
	-Z	Super premium parallize.
		If you have enough RAM and a multicore machine, you can speed
		up preprocessing time by setting the R_THREADS environment
		variable and proceeding with the usual preprocessing strategy
		(no need for ff nor do*/foreach packages).
		${bldred}Default 1 meaning no parallize, accept an integer
		larger than 1 to specify the numberof cores one want to use.${txtrst}
...
pma='FALSE'
runDE='FALSE'
rawr=1                #rows of raw signal
rawc=1				 #cols of raw signal
run='TRUE'
controlR=3
treatR=3
pvalue=0.05
...
while getopts "hf:s:i:M:m:n:a:r:R:L:Z:P:T:g:G:t:c:p:d:o:q:" OPTION
do
	case $OPTION in
		h)
			usage
...
if ($install){
	...
	biocLite(c("oligo"))
	if (${parallel} > 1) {
		install.packages("doMC", repo="http://cran.us.r-project.org")
	}
}
...
if (${super_p} > 1){
	Sys.setenv(R_THREADS=${super_p})
}

library('affy')
library("oligo")
print("##reading $file")
celFiles <- list.celfiles("$file", full.names=TRUE)

print('##reading cel files')
abc <- read.celfiles(celFiles)
