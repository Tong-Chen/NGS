...
This script is used to perform DE gene analysis using edgeR.
...
${txtbld}OPTIONS${txtrst}:
	...
	-s	Sample file ${bldred}[A multiple columns file with header line, 
		...
		CHECK ABOVE FOR DETAILS
		${txtrst}
	-S	A two column file specifying the length of each transcript.
...
if ("${compare_mode}" == "pairwise") {
	...
	#lrt.2vs1 <- glmLRT(fit, coef=3)
	#lrt.2vs1 <- glmLRT(fit, contrast=c(0, 1,0))
	#lrt.3vs2 <- glmLRT(fit, contrast=c(0, 0,1))
	#lrt.3vs2 <- glmLRT(fit, contrast=c(0,-1,1))
	
	## QL F-test is preferred as it reflects the uncertainty in estimating 
	## the dispersion for each gene. It provides more robust and reliable
	## error rate control when the number of replicates is small.
	#fit <- glmQLFit(y, design)
	#lrt.2vs1 <- glmQLFTest(fit, coef=2)
	#lrt.2vs1 <- glmQLFTest(fit, coef=3)
	#lrt.2vs1 <- glmQLFTest(fit, contrast=c(0, 1,0))
	#lrt.3vs2 <- glmQLFTest(fit, contrast=c(0, 0,1))
	#lrt.3vs2 <- glmQLFTest(fit, contrast=c(0,-1,1))
