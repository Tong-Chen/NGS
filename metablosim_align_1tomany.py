...
def alignTomain(primaryMZd, primaryMZd_keys, secondaryMZd, primary_base_rt, primary_common_rt, sec_base_rt, sec_common_rt, retention_time_sd, mz_ratio_sd, matchD, label):
    '''
    ...
    for secmz, primarymzL in sec2priD.items():
        ...
        for sec_label, sec_rt in secondaryMZd[secmz]:
            ...
            if primarymzL:
                ...
                if abs(closest_value - sec_rt_ratio) <= retention_time_sd:
...
#-------------------------alignTomain----------------
def outputMatchship_old(primaryMZd_labels, matchD, secondaryLabelL):
    '''
    matchD = {'primarymz_label': {samp_label1:[secondary_mzL, third_mzL, ..], 
                                  samp_label2:[forth_mzL]}, 
              'primarymz_label2':{samp_label3:[forth_mzL]}}
...
def main():
    ...
    for label, secondary,sec_base_rt, sec_common_rt in zip(secondaryLabelL, secondaryL, base_peak_rtL[1:], common_peak_rtL[1:]):
        ...
        alignTomain(primaryMZd, primaryMZd_keys, secondaryMZd, primary_base_rt, primary_common_rt, sec_base_rt, sec_common_rt, retention_time_sd, mz_ratio_sd, matchD, label)
    ...
    outputMatchship(primaryMZd_labels, matchD,secondaryLabelL)
    ###--------multi-process------------------
