...
desc = '''
Program description:
    This is designed to extract special rows as well as rename rows from matrix.
'''
...
def cmdparameter(argv):
    ...
    parser.add_option("-k", "--name-file", dest="name_file",
    ...
    parser.add_option("-R", "--rename-row", dest="rename_row",
        default = 0,
        help="Specify which column should be used as alternative row names to rename numerical matrix (given to -i) column files. Default 0 meaning no rename operation.")
    parser.add_option("-s", "--skip-line", dest="skip_line",
        type='int', default=0, 
        help="Skip the first X items in <name_file>. Normally 1 to skip header item. Default keep all items.")
    parser.add_option("-O", "--matrix-order", dest="matrix_order", default=False, 
        action="store_true", 
        help="Order rows for output as rows in <name_file> (default). Specify to keep order of rows as in original <filein>.")
...
def main():
    ...
    keep_colL = [line.strip().split('\t')[keep_col] for line in open(name_file)]
    keep_colL = keep_colL[skip_line:]
    #print >>sys.stderr, keep_colL
    matrix_order = options.matrix_order
    rename_row = int(options.rename_row) - 1
    if rename_row >= 0:
        rename_rowL = [line.strip().split('\t')[rename_row] for line in open(name_file)]
        rename_rowL = rename_rowL[skip_line:]
        renameD = dict(zip(keep_colL, rename_rowL))
    else:
        rename_rowL = []
    #fileL = re.split(r'[, ]*', file.strip())
    #if options.rename_rows != "None":
    #    label = options.label
    ...
    if matrix_order:
        matrix = matrix[matrix.index.isin(keep_colL)]
    else:
        matrix = matrix.loc[keep_colL,:]

    if rename_rowL:
        matrix.rename(index=renameD, inplace=True)
        #matrix.columns = rename_rowL
