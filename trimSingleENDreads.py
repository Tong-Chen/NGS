...
This is used to trim single-end reads in fastq format. 
...
def cmdparameter(argv):
    ...
    parser.add_option("-i", "--input-file", dest="filein",
        metavar="FILEIN", help="This is not used. A file containing \
the names of reads you want to trim and the ends they belong to. \
Such as reads_name1\nreads_name2")
    parser.add_option("-p", "--prefix", dest="prefix",
        metavar="reads", help="For file name <reads.fq>, <reads> \
...
def main():
    ...
    fq = options.prefix + '.' + suffix
    out_fq = options.prefix + '.trim.' + suffix
    first_base = int(options.first_base) - 1
    if options.last_base:
        last_base = int(options.last_base)
    else:
