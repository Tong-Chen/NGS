...
Functional description:
    This is designed to integrate the results of <runGOseq.pl> with multiple or single factor input.
...
def getTopTerm(type, prefix, top, *file):
    ...
    #--Get full-----------------------------------------------------------    
    file_out = open(file_out_n+'.xls', 'w')
    print >>file_out, "Sample\tGO\tTerm\tneg_log10pvalue\tCount\tFDR\tGene"
    for single in file:
        for line in open(single):
            if line.find("\t"+type) != -1:
...
def getTopTermFull(type, prefix, top, *file):
    #maxLen = 70
    file_out_n = prefix+"."+type+'_GO.xls'
    file_out = open(file_out_n, 'w')
    print >>file_out, "Sample\tGO\tTerm\tneg_log10pvalue\tCount\tFDR\tGene"
    for single in file:
        count = 0
