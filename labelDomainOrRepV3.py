...
===========================================================
This is version 3.
1. Add isoforms with multiple alignment
...
[-r rep] [-f seqrep] [-l locus]"
    ...
    parser.add_option('-m', '--multi-align', dest='seqalign',
            metavar=False, 
            help="Add multiple alignment of isoformer sequences",
            action = 'store_true')
    parser.add_option('-b', '--isoformerID', dest="isoformerID",
            metavar="ISOFORMID", help="All isoformer ")
...
#--------------------------------------------------------
def multipleAlign(id):
    print r'\section{'+id+'}'
    print r'\begin{texshade}{isoformers/'+id+'.aln'+'}'
    print r'''\seqtype{p}
...
To different application, tcoffee and isoformers should be changed"
    ...
    if options.repfile != None:
        ...
        isRep = 1
    if options.seqalign and options.isoformerID != None:
        isoformerL = [line[:-3] for line in
            open(options.isoformerID)]
        isoformerL = set(isoformerL)
        useIso = 1
        already = []
    ...
    for id in locusList:
        if id not in seqdict:
            ...
            if useIso and id[:-2] in isoformerL and\
                id[:-2] not in already:
                already.append(id[:-2])
                multipleAlign(id[:-2])
            #----------for multiple alignment-----------------
